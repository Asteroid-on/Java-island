...
- ✅ 天气详情：手动刷新按钮（刷新图标 + 悬停高亮，刷新期间禁用防重复请求）、右上角返回图标点击收起面板
- ✅ 湿度/空气质量/紫外线三信息等间距排列，多日预报图标旁显示中文天气状况
- ✅ 城市名精确到区级（高德逆地理编码，显示"市 区"，缓存 10 分钟防重复请求，失败回退市级）
- ✅ 天气数据每小时定时刷新 + 手动刷新双模式，双源并行拉取合并推送
- ✅ 渲染性能优化：绘制资源帧间缓存（字体/度量/渲染缓存零每帧分配）、可见区间遍历、动画透明度色板、HiDPI 图标多分辨率超采样；完整绘制一帧 p95 0.039ms
- ✅ 新增天气绘制性能基准测试（WeatherPaintPerfTest）

### v1.0.1
- ✅ 暂停时保持显示暂停位置歌词行
- ✅ 多屏自适应：主岛按鼠标所在屏定位与顶部触发
- ✅ 修复封面同步/自动弹出漂移/高 DPI 锯齿，移除英语语言选项
